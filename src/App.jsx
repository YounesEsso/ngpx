...
export default function App(){
  ...
  const loadProgress=async(u)=>{
    try{
      ...
      // On failure set default progress and continue — don't sign out
      setProgress(defaultProgress);
    }
  };
  ...
  useEffect(()=>{
    ...
    supabase.auth.getSession().then(({data:{session}})=>{
      ...
      // If session exists, onAuthStateChange will fire SIGNED_IN and handle it
    });

    return()=>subscription.unsubscribe();
  },[]);

  // Safety: if still loading after 8s, go to landing
  useEffect(()=>{
  ...
  if(appView==="login")return <LoginPage onBack={()=>setAppView("landing")} onSuccess={()=>setAppView("loading")}/>;
  ...
  // appView==="app" but user not loaded yet — show loading briefly
  if(appView==="app"&&!user){
    return(
      <div className="min-h-screen bg-black flex items-center justify-center font-mono text-sm">
        <div className="text-lime-400 animate-pulse">▸ LOADING...</div>
