...
const defaultProgress={placed:false,level:1,completed:{},triviaHigh:0,totalAnswered:0,totalCorrect:0,timeSpentSeconds:0,currentModule:null,currentQuestion:0,currentAnswers:[],placementIdx:0,placementPicks:[]};
        ...
        <Panel title={`USER PROGRESS // ${users.length} ACCOUNTS`} accent="fuchsia">
          {loading?(<div className="font-mono text-xs text-zinc-500 animate-pulse">Loading users...</div>):users.length===0?(<div className="font-mono text-xs text-zinc-500">No users yet.</div>):(
            <div className="font-mono text-xs overflow-x-auto">
              <div className="grid grid-cols-12 gap-2 px-2 py-1 border-b border-zinc-800 text-zinc-500 uppercase tracking-wider min-w-[800px]">
                <div className="col-span-3">User</div><div className="col-span-1 text-center">Level</div><div className="col-span-2 text-center">Placement</div><div className="col-span-1 text-center">Modules</div><div className="col-span-2 text-center">Accuracy</div><div className="col-span-1 text-center">Time</div><div className="col-span-2 text-center">Last seen</div>
              </div>
              {users.map(u=>{
                ...
                const placementLabel=u.placed?"Complete":placementDone>0?`Q${placementDone}/${placementTotal}`:"Not taken";
                const placementColor=u.placed?"text-lime-400":placementDone>0?"text-amber-400":"text-zinc-600";
                return(
                  <div key={u.id} onClick={()=>setSelected(selected?.id===u.id?null:u)} className="grid grid-cols-12 gap-2 px-2 py-2 border-b border-zinc-900 hover:bg-zinc-900/50 cursor-pointer min-w-[800px]">
...
function MainApp({user,progress,setProgress,onSignOut,onBackToAdmin,onGoHome}){
  ...
  const saveProgress=async(p)=>{
    await supabase.from("progress").upsert({
      ...
      placement_idx:p.placementIdx||0,placement_picks:p.placementPicks||[],
  ...
  const handlePlacementComplete=async(level,score,byLevel,byTopic)=>{
    ...
    const updated={...progressRef.current,placed:true,level,placementIdx:0,placementPicks:[]};
...
function ModuleView({moduleId,state,onComplete,onBack,onSaveQuizState}){
  ...
  // Deterministic shuffle based on moduleId — same order every time for same module
  // This ensures saved qIdx matches the correct question after re-login
  const shuffledQuestions=useMemo(()=>{
    if(!mod)return[];
    // Use moduleId as seed for consistent order
    const seed=mod.id.split('').reduce((a,c)=>a+c.charCodeAt(0),0);
    const qs=mod.questions.map((q,origIdx)=>({...shuffleQuestion(q),origIdx}));
