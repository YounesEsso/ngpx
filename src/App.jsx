...
function SignUpPage({onBack,onSuccess}){
  ...
  const emailLooksValid=/^[^\s@]+@engie\.com$/i.test(email.trim());

  const submit=async()=>{
    setError("");
    const trimmedEmail=email.trim().toLowerCase();
    if(!emailLooksValid){setError("Only @engie.com emails can sign up. Contact your administrator if you need access.");return;}
  ...
  return(
    <div className="min-h-screen bg-black flex items-center justify-center px-4">
      <div className="w-full max-w-sm">
        ...
        <Panel accent="lime" title="CREATE ACCOUNT">
          {done?(
            <div className="space-y-3 font-mono text-center py-4">
              <div className="text-lime-400 text-sm">✓ Account created!</div>
              <div className="text-zinc-300 text-xs leading-relaxed">
                Check your email — you may need to confirm your address before logging in.
              </div>
              <div className="pt-2">
                <Btn onClick={onSuccess} variant="primary">▸ GO TO LOGIN</Btn>
              </div>
            </div>
          ):(
            <div className="space-y-3 font-mono">
              <div>
                ...
                <input type="email" value={email} onChange={e=>setEmail(e.target.value)} className={`w-full bg-zinc-900 border text-zinc-100 px-3 py-2 text-xs font-mono focus:outline-none ${email&&!emailLooksValid?"border-rose-700 focus:border-rose-500":"border-zinc-700 focus:border-lime-500"}`} placeholder="firstname.lastname@engie.com"/>
                {email&&emailLooksValid&&<div className="text-lime-500 text-[10px] mt-1">✓ Engie email recognized</div>}
                {email&&!emailLooksValid&&<div className="text-rose-400 text-[10px] mt-1">Only @engie.com emails can sign up.</div>}
              </div>
              <div>
                <div className="text-[10px] text-zinc-500 uppercase tracking-wider mb-1">Full Name</div>
                <input value={fullName} onChange={e=>setFullName(e.target.value)} className="w-full bg-zinc-900 border border-zinc-700 text-zinc-100 px-3 py-2 text-xs font-mono focus:border-lime-500 outline-none" placeholder="Firstname Lastname"/>
              </div>
              <div>
                <div className="text-[10px] text-zinc-500 uppercase tracking-wider mb-1">Password (min. 8 chars)</div>
                <div className="relative">
                  <input type={showPwd?"text":"password"} value={password} onChange={e=>setPassword(e.target.value)} className="w-full bg-zinc-900 border border-zinc-700 text-zinc-100 px-3 py-2 pr-12 text-xs font-mono focus:border-lime-500 outline-none" placeholder="••••••••" onKeyDown={e=>e.key==="Enter"&&submit()}/>
        ...
        <div className="text-center mt-3 font-mono text-[10px] text-zinc-700">Restricted to @engie.com emails</div>
      </div>
    </div>
  );
}
      ...
      <main className="max-w-4xl mx-auto px-6 py-16">
        <div className="text-center mb-12">
          ...
          <div className="flex flex-col items-center gap-2">
            ...
            <div className="text-[10px] text-zinc-600 font-mono uppercase tracking-wider mt-1">For @engie.com emails</div>
          </div>
        </div>
...
function MainApp({user,progress,setProgress,onSignOut,onBackToAdmin,onGoHome}){
  ...
  useEffect(()=>{
    const interval=setInterval(async()=>{
      ...
      // C2 FIX: saveProgress now returns {success, error}; failures show banner via setSaveError
  ...
  const saveProgress=async(p)=>{
    // C2 FIX: wrap upsert in try/catch, return {success, error}.
    // Callers can check and react (warn user, retry, etc).
    try{
      ...
      // Clear any prior error on a successful save
  ...
  const handleModuleComplete=async(id,pct,total,correct)=>{
    ...
    const newScore=(prevScore!==undefined&&prevScore>=70)?Math.max(prevScore,pct):pct;
  ...
  return(
    <div className="min-h-screen bg-black text-zinc-100" style={{backgroundImage:"radial-gradient(circle at 20% 30%, rgba(132,204,22,0.04), transparent 50%), radial-gradient(circle at 80% 70%, rgba(245,158,11,0.04), transparent 50%)"}}>
      ...
      <main className="py-4">
        ...
        {showBetaBanner&&(
          <div className="max-w-2xl mx-auto px-4 mb-3">
            <div className="border border-amber-800 bg-amber-950/20 px-3 py-2 font-mono text-xs text-amber-300 flex justify-between items-center gap-3">
              <span>📢 You&apos;re testing the beta. Found something off? Use the <span className="text-amber-400 font-bold">🐛 Report bug</span> button in the header any time.</span>
        ...
        {view==="placement"&&<Placement onComplete={handlePlacementComplete} resumeIdx={progress.placementIdx} resumePicks={progress.placementPicks} onSavePlacement={handleSavePlacement} user={user}/>}
        ...
        {view==="module"&&activeModuleId&&<ModuleView moduleId={activeModuleId} state={{...progress,userId:user.id}} onComplete={handleModuleComplete} onBack={handleModuleBack} onSaveQuizState={handleSaveQuizState} user={user}/>}
...
// PLACEMENT
function Placement({onComplete,resumeIdx=0,resumePicks=[],onSavePlacement,user}){
    ...
    <div className="max-w-3xl mx-auto p-4 space-y-4">
      ...
      <div className="border border-amber-700 bg-amber-950/20 p-4 font-mono">
        ...
        <div className="text-zinc-500 text-[10px] mt-3 pt-3 border-t border-amber-900 leading-relaxed">
          ▸ Your level is set by the highest tier where you scored ≥50% on that tier's questions — not your overall percentage. You may have a high overall score but miss enough questions at a specific tier to land below it.
        </div>
      </div>
...
// MODULE VIEW
function ModuleView({moduleId,state,onComplete,onBack,onSaveQuizState,user}){
...
function TriviaMode({state,onUpdateHigh,onBack}){
  ...
  const next=()=>{
    if(idx===pool.length-1){setPhase("done");onUpdateHigh(score);}
    else{setIdx(idx+1);setSelected(null);setShowAnswer(false);setTimeLeft(15);setLastBreakdown(null);}
  };
...
function DisputeModal({user,context,moduleId,question,userAnswerIdx,onClose}){
  ...
  const submit=async()=>{
    ...
    try{
      ...
      if(e){setError(e.message||"Could not submit. Try again.");setLoading(false);return;}
      setDone(true);
      setLoading(false);
