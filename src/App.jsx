...
const defaultProgress={placed:false,level:1,completed:{},triviaHigh:0,totalAnswered:0,totalCorrect:0,timeSpentSeconds:0,currentModule:null,currentQuestion:0,currentAnswers:[]};
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-12">
          {Object.entries(CURRICULUM).map(([lvl,data])=>(
            <div key={lvl} className="border border-zinc-800 bg-zinc-950/60 p-4 font-mono">
              <div className="flex justify-between items-start mb-2"><Pill color={["lime","lime","amber","amber","cyan","fuchsia"][parseInt(lvl)-1]||"zinc"}>{data.tag}</Pill><span className="text-[10px] text-zinc-600">{data.grade}</span></div>
          ...
          {loading?(<div className="font-mono text-xs text-zinc-500 animate-pulse">Loading users...</div>):users.length===0?(<div className="font-mono text-xs text-zinc-500">No users yet.</div>):(
            <div className="font-mono text-xs overflow-x-auto">
              <div className="grid grid-cols-12 gap-2 px-2 py-1 border-b border-zinc-800 text-zinc-500 uppercase tracking-wider min-w-[700px]">
                <div className="col-span-3">User</div><div className="col-span-1 text-center">Level</div><div className="col-span-2 text-center">Modules</div><div className="col-span-2 text-center">Accuracy</div><div className="col-span-2 text-center">Time</div><div className="col-span-2 text-center">Last seen</div>
              </div>
              {users.map(u=>{
                ...
                return(
                  <div key={u.id} onClick={()=>setSelected(selected?.id===u.id?null:u)} className="grid grid-cols-12 gap-2 px-2 py-2 border-b border-zinc-900 hover:bg-zinc-900/50 cursor-pointer min-w-[700px]">
                    ...
                    <div className="col-span-2 text-center text-zinc-300">{cc}/{totalModules}</div>
...
function MainApp({user,progress,setProgress,onSignOut,onBackToAdmin}){
  const [view,setView]=useState(()=>{
    if(progress.currentModule)return"module";
    if(progress.placed)return"dashboard";
    return"welcome";
  });
  const [activeModuleId,setActiveModuleId]=useState(progress.currentModule||null);
  ...
  return(
    <div className="min-h-screen bg-black text-zinc-100" style={{backgroundImage:"radial-gradient(circle at 20% 30%, rgba(132,204,22,0.04), transparent 50%), radial-gradient(circle at 80% 70%, rgba(245,158,11,0.04), transparent 50%)"}}>
      ...
      <main className="py-4">
        ...
        {view==="module"&&activeModuleId&&<ModuleView moduleId={activeModuleId} state={progress} onComplete={handleModuleComplete} onBack={()=>{persist({...progress,currentModule:null,currentQuestion:0,currentAnswers:[]});setView("dashboard");}} onSaveQuizState={async(modId,qIdx,ans)=>{const updated={...progress,currentModule:modId,currentQuestion:qIdx,currentAnswers:ans,totalAnswered:progress.totalAnswered+(ans.length>progress.currentAnswers.length?1:0),totalCorrect:progress.totalCorrect+(ans.length>progress.currentAnswers.length&&ans[ans.length-1]?1:0)};await persist(updated);}}/>}
    ...
    <div className="max-w-2xl mx-auto p-4">
      <Panel title={`PLACEMENT // ${idx+1} / ${PLACEMENT.length}`} accent="amber">
        <div className="font-mono space-y-4">
          ...
          <div className="text-xs text-zinc-500 flex justify-between"><span>L{q.lvl} · {q.topic}</span><span>NO HINTS // NO BACKTRACK</span></div>
          <div className="text-zinc-100 text-sm">{q.q}</div>
          <div className="space-y-2">
            {q.options.map((o,i)=>(<button key={i} onClick={()=>setSelected(i)} className={`block w-full text-left p-2.5 border font-mono text-xs ${selected===i?"border-amber-400 bg-amber-400/10 text-amber-100":"border-zinc-800 text-zinc-300 hover:border-amber-700"}`}><span className="text-zinc-500 mr-2">[{String.fromCharCode(65+i)}]</span>{o}</button>))}
          </div>
          <div className="pt-2 flex justify-end"><Btn onClick={submit} variant="primary" disabled={selected===null}>{idx===PLACEMENT.length-1?"FINISH":"NEXT"} <ChevronRight size={12} className="inline"/></Btn></div>
        </div>
      </Panel>
    </div>
  );
...
function ModuleView({moduleId,state,onComplete,onBack,onSaveQuizState}){
  ...
  const [phase,setPhase]=useState(resumeQ>0?"resume":"lesson");
  ...
  const submit=()=>{
    if(selected===null)return;
    setShowAnswer(true);
    const correct=selected===mod.questions[qIdx].answer;
    const newAnswers=[...answers,correct];
    setAnswers(newAnswers);
    onSaveQuizState(moduleId,qIdx,newAnswers);
  };
  const next=()=>{
    if(qIdx===mod.questions.length-1){const c=answers.filter(a=>a).length;const pct=Math.round((c/mod.questions.length)*100);onComplete(mod.id,pct,mod.questions.length,c);setPhase("done");onSaveQuizState(null,0,[]);}
  ...
  if(phase==="resume")return(
    <div className="max-w-2xl mx-auto p-4 space-y-3">
      <button onClick={onBack} className="font-mono text-xs text-zinc-500 hover:text-lime-400 flex items-center gap-1"><ArrowLeft size={12}/> BACK</button>
      <Panel title={`${mod.id} // RESUME`} accent="amber">
        <div className="font-mono text-zinc-300 text-sm space-y-3">
          <div className="text-amber-400">▸ You left this quiz at question {resumeQ+1}/{mod.questions.length}</div>
          <div className="text-zinc-400 text-xs">You answered {resumeA.length} question{resumeA.length!==1?"s":""} — {resumeA.filter(Boolean).length} correct so far.</div>
          <div className="flex gap-2 mt-4">
            <Btn onClick={()=>setPhase("quiz")} variant="primary">▸ RESUME FROM Q{resumeQ+1}</Btn>
            <Btn onClick={()=>{setQIdx(0);setAnswers([]);setPhase("quiz");}} variant="ghost">RESTART</Btn>
          </div>
        </div>
      </Panel>
    </div>
