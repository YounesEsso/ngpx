  ...
  return(
    <div className="min-h-screen bg-black text-zinc-100" style={{backgroundImage:"radial-gradient(circle at 20% 30%, rgba(132,204,22,0.04), transparent 50%), radial-gradient(circle at 80% 70%, rgba(245,158,11,0.04), transparent 50%)"}}>
      ...
      <main className="py-4">
        ...
        {view==="module"&&activeModuleId&&<ModuleView moduleId={activeModuleId} state={progress} onComplete={handleModuleComplete} onBack={handleModuleBack} onSaveQuizState={handleSaveQuizState}/>}
...
function Placement({onComplete}){
  ...
  // Shuffle all placement questions once on mount
  const shuffled=useMemo(()=>PLACEMENT.map(q=>shuffleQuestion(q)),[]);
  ...
  const advance=()=>{
    ...
    }else{setPicks(next);setSelected(null);setShowAnswer(false);setIdx(idx+1);}
  };
...
function ModuleView({moduleId,state,onComplete,onBack,onSaveQuizState}){
  ...
  // Shuffle questions once on mount — stable for this session
  const shuffledQuestions=useMemo(()=>mod?mod.questions.map(q=>shuffleQuestion(q)):[],[mod?.id]);
    ...
    <div className="max-w-2xl mx-auto p-4 space-y-3">
      ...
      <Panel title={`${mod.id} QUIZ // ${qIdx+1}/${shuffledQuestions.length}`} accent="cyan">
        <div className="font-mono space-y-3">
          ...
          {resumeQ>0&&qIdx===resumeQ&&<div className="text-[10px] text-amber-400 uppercase tracking-wider">▸ Continuing from where you left off — answers are locked</div>}
          <div className="text-zinc-100 text-sm">{q.q}</div>
          <div className="space-y-2">
            {q.options.map((o,i)=>{
              const ic=i===q.answer,is=i===selected;
