...
const CURRICULUM = {
  1:{name:"FOUNDATIONS",grade:"Elementary (G1-3)",tag:"L1",modules:[
    {id:"1a",title:"What is Electricity?",lesson:"Electricity = flow of electrons through a wire. Generated at a power plant, sent over high-voltage transmission lines, then through smaller distribution lines to your home. Critical rule: bulk electricity cannot be cheaply stored — it must be made the moment it's used. This drives the entire power-market design.",questions:[
      {q:"What flows through a wire to make current?",options:["Water molecules pushed through the wire","Electrons moving directionally through wire","Compressed air pressure waves in the wire","Photons emitted by the voltage source"],answer:1,explain:"Electrons = charged particles whose motion = current."},
...
function MainApp({user,progress,setProgress,onSignOut,onBackToAdmin,onGoHome}){
  ...
  const handleModuleComplete=async(id,pct,total,correct)=>{
    ...
    await persist({...p,level:nl,completed:nc,totalAnswered:p.totalAnswered+total,totalCorrect:p.totalCorrect+correct,currentModule:null,currentQuestion:0,currentAnswers:[]});
  };
...
export default function App(){
  ...
  // Kill loading screen after 10s no matter what — sign out and go to login
  useEffect(()=>{
    const timer=setTimeout(async()=>{
      if(appView==="loading"){
        console.warn("Loading timeout — resetting session");
        await supabase.auth.signOut();
        setUser(null);setProgress(defaultProgress);setAppView("login");
      }
    },6000);
  ...
  const loadProgress=async(u)=>{
    try{
      // Race loadProgress against a 8s timeout — never hang forever
      ...
      // On timeout or error: sign out and go to login — clean state
      console.error("loadProgress failed:",e.message);
      await supabase.auth.signOut();
      setUser(null);setProgress(defaultProgress);setStudentView(false);setAppView("login");
    }
  };
